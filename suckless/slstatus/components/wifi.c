...
wifi_essid(const char *interface)
{
	static char output[128]; // holds icon + ESSID
	static char id[IW_ESSID_MAX_SIZE + 1];
	const char *wifi_connected_icon = " ";
	const char *wifi_disconnected_icon = "󰖪 ";
	int sockfd;
	struct iwreq wreq;
	...
	esnprintf(output, sizeof(output), "%s %s", wifi_connected_icon, id);
	return output;
}
