...
bool cg_remove_image(arg_t _) {
    ...
    // prompt string
    if (marked_count > 0)
        snprintf(prompt, sizeof(prompt), "Delete %d image%s?", marked_count, marked_count == 1 ? "" : "s");
    else
        snprintf(prompt, sizeof(prompt), "Delete this image?");

    // Build dynamic dmenu args
    const char *delete_prompt[] = {
        "dmenu", "-i", "-p", prompt, NULL
    };

    if (spawn(&rfd, &wfd, 0, (char **)delete_prompt) < 0)
        return false;

    dprintf(wfd, "yes\nno\n");
    close(wfd);

    if ((n = read(rfd, response, sizeof(response) - 1)) <= 0) {
        close(rfd);
        return false;
    }
    close(rfd);

    char *eol = memchr(response, '\n', n);
    if (eol)
        *eol = '\0';
    else
        response[n] = '\0';
    ...
    if (marked_count > 0) {
        ...
        if (remove(files[fileidx].path) != 0) {
            return false;
        }
        remove_file(fileidx, true);
        deleted_any = true;
...
// Function to copy the file path to the clipboard using xclip
bool cg_copy_path(arg_t _) {
    if (!files || !files[fileidx].path)
        return false;

    // Command to run xclip
    const char *const copy_prompt[] = {
        "xclip", "-selection", "clipboard", "-in", NULL
    };

    // Spawn a new process to copy the file path to the clipboard using xclip
    int rfd, wfd;
    if (spawn(&rfd, &wfd, 0, (char **)copy_prompt) < 0)
        return false;

    // Write the file path to xclip
    dprintf(wfd, "%s\n", files[fileidx].path);
    close(wfd);
