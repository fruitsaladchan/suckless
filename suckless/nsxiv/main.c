...
static void update_info(void)
{
	...
	if (mode == MODE_THUMB) {
		...
		bar_put(r, "%dx%d" BAR_SEP, img.w, img.h); //image resoultion
		{ //show file size
			struct stat st;
			if (stat(files[fileidx].path, &st) == 0) {
				...
				bar_put(r, "%.1f%s" BAR_SEP, size, units[unit]);
			}
		}
		{ // show compression mode (lossy/lossless)
			magic_t magic = magic_open(MAGIC_MIME_TYPE);
			if (magic) {
				if (magic_load(magic, NULL) == 0) {
					const char *mime = magic_file(magic, files[fileidx].path);
					if (mime) {
						const char *compression = NULL;
						if (strstr(mime, "jpeg") || strstr(mime, "webp"))
							compression = "Lossy";
						else if (strstr(mime, "png") || strstr(mime, "gif") ||
								strstr(mime, "bmp") || strstr(mime, "tiff") || strstr(mime, "x-icon") ||
								strstr(mime, "heif") || strstr(mime, "heic"))
							compression = "Lossless";
						else
							compression = "Unknown";

						bar_put(r, "%s" BAR_SEP, compression);
					}
				}
				magic_close(magic);
			}
